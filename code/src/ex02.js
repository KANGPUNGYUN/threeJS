import * as THREE from 'three'; // 3js 코어 파일
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js'; //3js 확장된 기술, 외부모델을 불러오는 기능
import gsap from 'gsap'; // 외부 자바스크립트 라이브러리
...
export default function example() {
	...
	function setSection() {
		scrollY = window.scrollY; // window.pageYOffset; 이건 과거에 사용했던 메서드이다
		// console.log(scrollY)
		...
		if (newSection != currentSection) {
			...
			if (sampleModels[currentSection].mesh) { //로드가 안되었을때 에러가 나는 것을 방지하기 위해서
				gsap.from(
